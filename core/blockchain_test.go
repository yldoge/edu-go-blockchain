...
func TestAddBlock(t *testing.T) {
	...
	assert.NotNil(t, bc.AddBlock(randomBlock(uint32(chainHeight)+1, types.Hash{})))
	// Add an existed block should fail
	assert.NotNil(t, bc.AddBlock(randomBlockWithSignature(t, uint32(100), types.Hash{})))
}
...
func TestAddBlockToHeight(t *testing.T) {
	...
	assert.NotNil(t, bc.AddBlock(randomBlock(3, types.Hash{})))
}
